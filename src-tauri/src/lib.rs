...
pub fn run() {
    // WebKitGTK's DMABUF renderer aborts with "EGL_BAD_ALLOC" on some Linux
    // GPU/driver stacks (notably NVIDIA), so the app crashes or shows a blank
    // window. Disabling it falls back to a working render path and is harmless
    // on setups that already worked. Respect an explicit user override.
    #[cfg(target_os = "linux")]
